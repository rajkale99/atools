...
for var in "$@"; do
    ...
    repo_desc=dump
    ORG="rajkale99"
    curl https://api.gitlab.com/user/repos\?access_token=$LAB_TOKEN -d '{"name": "'"$repo"'","description": "'"$repo_desc"'","private": false,"has_issues": true,"has_projects": false,"has_wiki": true}' > /dev/null 2>&1

    git init > /dev/null 2>&1
    git branch -m $BRANCH > /dev/null 2>&1
    git remote add origin https://gitlab.com/$ORG/${repo,,}.git > /dev/null 2>&1
    ...
    git push https://$LAB_TOKEN@gitlab.com/$ORG/${repo,,}.git $BRANCH > /dev/null 2>&1
    ...
    [[ -d vendor/ ]] && git push https://$LAB_TOKEN@gitlab.com/$ORG/${repo,,}.git $BRANCH > /dev/null 2>&1
    echo -e "Dumping apps"
